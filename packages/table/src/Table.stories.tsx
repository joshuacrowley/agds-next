...
import { TableHeader, SortableTableHeader } from './TableHeader';
...
const data = [
	...
	{
		submissionDate: '2022-06-08',
		id: '04bc33ac-134c-4edb-a221-81d345a891ec',
		type: 'Establishment registration',
	},
	{
		submissionDate: '2022-06-07',
...
export const WithSort: ComponentStory<typeof Table> = (args) => {
	type DataFields = keyof typeof data[number];

	const [currentlySortedColumn, setCurrentlySortedColumn] =
		useState<DataFields>('submissionDate');
	const [sortDir, setSortDir] = useState<'asc' | 'dsc'>('dsc');

	const sortedData = data.sort((a, b) => {
		const aValue = a[currentlySortedColumn] || 0;
		const bValue = b[currentlySortedColumn] || 0;
			...
			<Table {...args}>
				...
				<TableHead>
					<tr>
						<SortableTableHeader
							sortDir={sortDir}
							isActivelySortedBy={currentlySortedColumn === 'submissionDate'}
							onSortChange={(direction) => {
								setCurrentlySortedColumn('submissionDate');
						...
						<SortableTableHeader
							sortDir={sortDir}
							isActivelySortedBy={currentlySortedColumn === 'id'}
							onSortChange={(direction) => {
								setCurrentlySortedColumn('id');
								setSortDir(direction);
							}}
						>
							Name
						...
						<SortableTableHeader
							sortDir={sortDir}
							isActivelySortedBy={currentlySortedColumn === 'type'}
							onSortChange={(direction) => {
								setCurrentlySortedColumn('type');
								setSortDir(direction);
							}}
				...
				<TableBody>
					{sortedData.map(({ id, submissionDate, type }) => (
						<tr key={id}>
							<TableCell>{submissionDate || 'Not yet submitted'}</TableCell>
							<TableCell>
								<TextLink href={`#${id}`}>{id}</TextLink>
